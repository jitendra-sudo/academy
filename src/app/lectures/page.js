            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {lecture.instructor && (
                <span className="text-gray-400 text-xs flex items-center gap-1">
                  <span>👤</span> {lecture.instructor}
                </span>
              )}
              {lecture.duration && (
                <span className="text-gray-400 text-xs font-mono">⏱ {lecture.duration}</span>
              )}
            </div>
          </div>
          <button
...
export default function LecturesPage() {
  ...
  const [whatsapp, setWhatsapp] = useState("+919003190030");

  // Fetch CMS contact settings for dynamic WhatsApp number
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => r.json())
      .then((d) => { if (d.success && d.data?.contact?.whatsapp) setWhatsapp(d.data.contact.whatsapp); })
      .catch(() => {});
  }, []);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        ...
        <section className="bg-gradient-to-br from-[#0f172a] via-[#1e3a8a] to-[#1d4ed8] py-20 pt-28 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            ...
            <p className="text-blue-200 text-lg max-w-2xl mx-auto mb-8">
              Access free video lectures on History, Geography, Polity, Economy, Current Affairs, Ethics, and more — curated by Mentors Merits Academy.
            </p>
            {/* Search */}
        ...
        <div className="container mx-auto px-4 py-10">
          ...
          {featured.length > 0 && !search && activeCourse === "All" && activeSubject === "All" && (
            <section className="mb-12">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {featured.map((lec) => (
                  <LectureCard key={lec.id} lecture={lec} onPlay={setPlaying} />
                ))}
              </div>
            </section>
          )}
          ...
          <div className="mt-16 bg-gradient-to-br from-[#0f172a] to-[#1e3a8a] rounded-3xl p-8 text-center text-white">
            ...
            <p className="text-blue-200 mb-6">Join thousands of students achieving their UPSC dreams with Mentors Merits Academy.</p>
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <a href={`https://wa.me/${whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="bg-green-500 hover:bg-green-400 text-white font-bold px-6 py-3 rounded-xl transition-all flex items-center gap-2">
      ...
      <footer className="bg-[#0f172a] text-white py-8 text-center text-sm">
        <p className="text-gray-400">© 2026 Mentors Merits Academy. All rights reserved.</p>
